<template>
  <div class="slider">
    <div v-for="(doctor, index) in doctors" :key="doctor.id" class="doctor" :style="doctorStyle(index)" @click="handleClick(index)">
      <h1>{{ doctor.name }}</h1>
      <p>Lorem ipsum dolor sit amet consectetur, adipisicing elit. Facere magni magnam unde ipsam repudiandae explicabo expedita labore, sequi minus neque beatae voluptatum, quasi accusamus quia quis voluptas laborum ad! Ab totam doloribus, excepturi possimus rem vel quia fugit molestiae officiis!</p>
    </div>
...
export default {
  ...
  methods: {
    ...
    doctorStyle(index) {
      ...
      const transform = isActive ? 'none' : `translateX(<span class="math-inline">\{120 \* stt\}px\) scale\(</span>{1 - 0.2 * stt}) perspective(16px) rotateY(${isActive ? 0 : stt > 2 ? -1 : 1}deg)`;
    ...
    handleClick(index) {
      ...
      for(var i = this.activeIndex + 1; i < items.length; i++){
          stt++;
          items[i].style.transform = `translateX(<span class="math-inline">\{120\*stt\}px\) scale\(</span>{1 - 0.2*stt}) perspective(16px) rotateY(-1deg)`;
